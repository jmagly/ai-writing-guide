...
describe('normalizeSourceType (#1509) — folds the 3 drifting vocabularies', () => {
  ...
  it('maps the body "Source Type" field', () => {
    ...
    expect(normalizeSourceType({ bodySourceType: 'pcap-evidence' })).toBe('packet-evidence');
  });
...
describe('registry accessors', () => {
  it('lists canonical types and resolves their rules', () => {
    ...
    expect(types).toContain('packet-evidence');
    ...
    expect(getSourceType('packet-evidence')?.qualityRules).toBe('observational-evidence-no-grade');
