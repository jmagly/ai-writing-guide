...
research-complete agent definitions. Externalizing them keeps agent definitions
under the 16 KB subagent-dispatch ceiling (see the `few-shot-examples` rule and
#1587) while preserving the 2-3 few-shot floor — the examples still *exist*,
just here instead of bloating every system prompt.
...
- #1600 — debloat oversized research-complete agent definitions
