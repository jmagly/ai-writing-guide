# Campaign Orchestrator — Worked Examples

Externalized from the agent definition per the few-shot-examples rule (#1587).

These are the full multi-channel campaign template samples that back the
Campaign Orchestrator. The agent definition keeps the process, section
structure, launch checklist, and a single compact inline anchor; the full
templates live here.
