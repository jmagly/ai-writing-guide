...
**Key property**: AIWG has 15 named provider integrations. If it can also ingest from multiple marketplace sources, a compatible plugin can be projected onto each provider's supported artifact surfaces — install once, adapt per provider.
...
This means marketplace plugins automatically benefit from AIWG's multi-provider deployment — install once from any marketplace, then project onto the supported surfaces of all 15 named provider integrations.
