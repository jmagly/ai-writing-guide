...
OpenClaw is the **provider** in this guide. Complete the [safe Node.js setup](../getting-started/install-node.md)
...
The deployment command refreshes AIWG's shared project context and prints a verification result. Restart OpenClaw if
the `aiwg use` output says a reload is required. After that, ask OpenClaw to verify AIWG by reporting the project
...
Success means OpenClaw names the intended project, follows the AIWG bootstrap into `WORKSPACE.md` and `AIWG.md` or the
provider-specific adapter, and produces a concrete review you can inspect. For advanced flags, compatibility notes,
and recovery details, see the [OpenClaw operational
reference](https://github.com/jmagly/aiwg/blob/main/docs/agents/providers/openclaw.md).
