...
The key signs both annotated release tags and commits to `main`. The release-tag signature is the cryptographic anchor — CI's `tools/ci/verify-signed-tag.sh` gate (#1299) fails any publish whose tag does not verify against a key in `.gitea/keys/maintainers.asc` or `.gitea/allowed_signers`. Commit signatures use the same key so GitHub renders the "Verified" badge once the key is registered to the maintainer's GitHub identity (with a secondary UID matching the GitHub no-reply email).
