...
The discovery audit that surfaced the cross-provider deployment gap exists because of the [`feedback_no_platform_generalization`](https://github.com/jmagly/aiwg/blob/main/.aiwg/studies/novice-user-adoption/README-feedback-loop.md) discipline: static analysis flags candidates, only field reports conclude breakage. The audit upgraded the entire matrix from `static-flagged` to `deployment-scripted` in a single cycle, then surfaced the rule-deployment gap as a concrete actionable finding. Without that discipline, the parity gap would have stayed hypothesized rather than measured.
