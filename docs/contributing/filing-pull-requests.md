...
**If `mode: direct`, do not open a PR.** Commit directly to `main` with `Closes #N` in the message. The [`delivery-policy`](../../agentic/code/addons/aiwg-utils/rules/delivery-policy.md) rule (kernel rule, always loaded) is authoritative.
...
The project [no-attribution rule](../../agentic/code/frameworks/sdlc-complete/rules/no-attribution.md) is universal across platforms. The commit/tag/PR body **must not** contain:
...
Use the template at [`.gitea/pull_request_template.md`](../../.gitea/pull_request_template.md) (or [`.github/pull_request_template.md`](../../.github/pull_request_template.md)). Required sections:
...
2. **Fix the underlying issue** — no `|| true`, no `continue-on-error: true`, no test deletion. See [`anti-laziness`](../../agentic/code/frameworks/sdlc-complete/rules/anti-laziness.md) and [`dev-pipeline-safety`](../../agentic/code/frameworks/sdlc-complete/rules/dev-pipeline-safety.md) rules.
...
- Update [`CHANGELOG.md`](../../CHANGELOG.md) if the change is user-visible — for releases, the [`flow-release`](../../agentic/code/frameworks/sdlc-complete/skills/flow-release/SKILL.md) skill handles this; for individual PRs that ship before the release, append to the unreleased section

---

## Release-time PRs

When the PR is part of a release sequence, the [`flow-release` skill](../../agentic/code/frameworks/sdlc-complete/skills/flow-release/SKILL.md) handles the orchestration. The PR itself follows the same rules as any other PR, but the release flow adds extra gates:
...
See [`flow-release`](../../agentic/code/frameworks/sdlc-complete/skills/flow-release/SKILL.md) for the full sequence. The `.aiwg/release.config` declares which gates apply.
...
- **`continue-on-error: true` on tests** — never silence the smoke detector. See [`dev-pipeline-safety`](../../agentic/code/frameworks/sdlc-complete/rules/dev-pipeline-safety.md).
- **Skipping CI green wait** — if CI fails, fix it before declaring done.
- **Bundling unrelated changes** — split into focused PRs.
- **Workarounds instead of root-cause fixes** — see [`anti-laziness`](../../agentic/code/frameworks/sdlc-complete/rules/anti-laziness.md) Rule 9: "real fix over workaround".
...
If the verification gates fail and you can't fix them in 3 attempts, **escalate** per the [`anti-laziness`](../../agentic/code/frameworks/sdlc-complete/rules/anti-laziness.md) recovery protocol. Don't take destructive shortcuts (skip tests, weaken assertions, etc.).
...
- **Templates**: [`.gitea/pull_request_template.md`](../../.gitea/pull_request_template.md), [`.github/pull_request_template.md`](../../.github/pull_request_template.md)
- **Rules**: [`delivery-policy`](../../agentic/code/addons/aiwg-utils/rules/delivery-policy.md) (kernel), [`no-attribution`](../../agentic/code/frameworks/sdlc-complete/rules/no-attribution.md) (kernel), [`anti-laziness`](../../agentic/code/frameworks/sdlc-complete/rules/anti-laziness.md), [`dev-pipeline-safety`](../../agentic/code/frameworks/sdlc-complete/rules/dev-pipeline-safety.md)
- **Release flow**: [`flow-release`](../../agentic/code/frameworks/sdlc-complete/skills/flow-release/SKILL.md)
