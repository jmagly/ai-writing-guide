...
The baseline `.aiwg/AIWG.md` file is generated by `aiwg init` and refreshed by `aiwg use` / `aiwg regenerate`. AIWG preserves operator-authored content outside managed blocks, then updates the `aiwg-context-finalization` block from `.aiwg/aiwg.config` so the file records configured providers, installed frameworks/addons, recorded deployments, and the discover-first protocol.

Root `AIWG.md` and provider-facing files such as `AGENTS.md`, `WARP.md`, `.hermes.md`, and `.github/copilot-instructions.md` are generated bridges. They may mirror or summarize this normalized context, but distributable skills and rules should reference `@.aiwg/AIWG.md` when they need the stable project-local source.
