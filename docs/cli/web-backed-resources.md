...
stable_id: aiwg.agent-reference.web-resources
---

# Web-Backed AIWG Resources

> **Audience: agents, scripts, and advanced operators.** General users should
> ask their agent to load a verified signed asset and follow
> [Signed Web Resources](../install/web-backed-resources.md).
...
| `Unsupported AIWG resource selector` | The selector is not an exact CalVer release, SemVer range, `sha256:<digest>` manifest selector, or channel name in this beta. | Use an exact version such as `2026.7.18`, a range such as `>=2026.7.18 <2026.8.0`, a manifest digest selector, or a channel such as `stable`, `latest`, `canary`, or `main`. |
