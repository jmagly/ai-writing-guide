...
<!--
EXPANDED TEMPLATE — use this template instead of the canonical 15-section
template when the paper warrants deeper analysis:
...
- Papers with significant limitations or open questions worth tracking explicitly
- Papers that demand a comparison-with-related-work section to be useful

The expanded template extends the canonical 15-section structure with these
additional sections (positioned where they best support the analysis):

  - Conceptual Framework      (after Summary; lays out theoretical scaffolding)
  - Benchmark Results         (after Architecture; the metrics tables)
  - Ablation Studies          (after Benchmark Results; what mattered)
  - Key Insights              (after Ablations; the "so what" interpretation)
  - Limitations and Risks     (after Key Insights; what the paper does NOT establish)
  - Comparison with Related Work  (after Cross-References; situated discussion)
  - Future Research Directions    (after Comparison; what comes next)

The legacy "expanded" convention (~125 corpus docs as of 2026-05-22) uses
unnumbered headings. This template uses numbered headings for consistency
with the canonical template — the section *content* is the load-bearing
distinction, not the numbering.
-->
...
Affects AIWG / cross-project components:
