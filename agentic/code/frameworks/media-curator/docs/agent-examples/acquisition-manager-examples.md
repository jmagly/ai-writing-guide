# Acquisition Manager — Worked Examples

Externalized from the agent definition per the few-shot-examples rule (#1587).

These are the concrete shell, JSON, and report samples that back the
Acquisition Manager's process. The agent definition keeps the prose, tables
(tool-selection matrix, retry matrix, extraction format selection), rules, and a
single compact inline anchor; the full samples live here.
...
## Format Selection Strategy (full)
...
## Network Mount Rules (Field-Tested) — full code samples
