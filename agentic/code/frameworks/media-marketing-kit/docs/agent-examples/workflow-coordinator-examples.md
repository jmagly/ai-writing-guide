# Workflow Coordinator — Worked Examples

Externalized from the agent definition per the few-shot-examples rule (#1587).

These are the full workflow template samples that back the Workflow
Coordinator. The agent definition keeps the process, section structure, and a
