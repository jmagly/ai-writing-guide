# Agent Worked-Examples Catalog (Media Marketing Kit)

Discoverable home for the few-shot worked examples that used to live inline in
media-marketing-kit agent definitions. Externalizing them keeps agent
definitions under the 16 KB subagent-dispatch ceiling (see the
`few-shot-examples` rule and #1587/#1600) while preserving the 2-3 few-shot
floor — the examples still *exist*, just here instead of bloating every system
prompt.
