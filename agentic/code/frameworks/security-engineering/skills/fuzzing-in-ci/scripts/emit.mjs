...
for (const lang of languages) {
  ...
  fs.writeFileSync(path.join(outRoot, ci, `${lang}.yaml`), recipe(lang, seconds));
}
fs.writeFileSync(path.join(outRoot, 'merge_corpus.sh'), '#!/usr/bin/env bash\nset -euo pipefail\nllvm-profdata merge "$@"\n');
fs.writeFileSync(path.join(outRoot, 'OSS-FUZZ.md'), '# OSS-Fuzz Integration\n\nAdd project.yaml, Dockerfile, and build.sh after confirming stable fuzz targets.\n');
