...
- **`aiwg refresh` auto-marks + cleans up the legacy registry** (#1054). `refreshHandler` now invokes `migrateLegacyRegistry()` early in the flow when an unmarked `.aiwg/frameworks/registry.json` is present, and `cleanupLegacyRegistry()` after re-deploy when the marker is present and ≥ 2 minor versions have elapsed. Closes the wire-up gap noted in commit `ee04622e` — `aiwg doctor` now flips the legacy-registry warning to an info-level note after a single refresh. Both calls are non-blocking; failures emit a stderr warning and continue. Dry-run shows the planned action without writing.
...
- **Legacy `.aiwg/frameworks/registry.json` deprecation marker + cleanup path** (#1047, epic #1033). `migrateLegacyRegistry` now writes a `_deprecatedMigratedAt` marker (preserving the original payload so a partial rollback is recoverable) after a successful first migration, and records the AIWG version that performed the migration. New `cleanupLegacyRegistry(projectDir, currentVersion)` deletes the legacy file when the marker is present AND ≥2 minor versions have elapsed since migration. New `checkLegacyRegistry(projectDir, currentVersion)` reports presence + eligibility without side effects. `aiwg doctor` now surfaces the legacy file's deprecation state (info-level when marked, warn-level when unmarked). The cleanup function is callable but not yet wired to a trigger event — an operator can `rm` the file manually after `aiwg doctor` reports it eligible. Wire-up to `aiwg refresh` is follow-on work under epic #1033.
