...
Source attribution: migrated from `CLAUDE.md`; checksum `9f7a7282bb32f44d6f04f4de0f61a415e71c2b0751b128cf1502476640aa3ad8`.
...
6. **Cut the tag via the wrapper — REQUIRED, never `git tag` by hand** — export the ci-aiwg reader creds (`source ~/.config/openbao/env; export BAO_CI_ROLE_ID="$(_openbao_cred ci-aiwg role-id)" BAO_CI_SECRET_ID="$(_openbao_cred ci-aiwg secret-id)"`), then `tools/release/cut-tag.sh <X.Y.Z>`. The wrapper runs its pre-tag checks (CalVer shape, `package.json` + `marketplace.json` lockstep, **CHANGELOG entry present**, **announcement file present**, release-signing key published in `.gitea/keys/maintainers.asc`), **sources the release-signing key from OpenBao** (`kv_internal/gpg/release-signing-key` via the `ci-aiwg` AppRole) into an ephemeral keyring, and signs with `-u <RELEASE_KEY_FINGERPRINT>` (default: `9292EFCBB0EA41BECEEFDAFA9C1B8CE0E0E09C33`, the dedicated CI `AIWG Release Signing <release@aiwg.io>` key — rekeyed 2026-07-12 from the retired `FE9272…` personal key, which stays in `maintainers.asc` for historical verification). Set `AIWG_RELEASE_SIGN_FROM_VAULT=0` for fork/offline signing with a locally-held key. **Bypassing this wrapper is how v2026.5.10 shipped without a CHANGELOG entry — don't repeat the failure.** See `docs/contributing/versioning.md` → "Signing your release tag".
7. **Push tag to Gitea** — `git push origin main --tags`. Triggers `gitea-release.yml` + `npm-publish.yml` (both gated on signed-tag verify).
8. **Mirror to GitHub** — `git push github main --tags`. **You must push the tag yourself** (don't rely on the mirror workflow to push it): GitHub's npm-publish workflow verifies the tag *signature*, and a tag pushed from the Gitea runner arrives peeled to a lightweight (unsigned) object that fails that gate. Your local `git push` preserves the annotated, signed tag. The Gitea tag push in step 7 then triggers `github-mirror.yml`, which **creates the GitHub Release** using `docs/releases/v<version>-announcement.md` as the body (reading the `GH_ACCESS_TOKEN` Gitea Actions secret) — so **no manual `gh release create` is needed**. The mirror's main-push is best-effort and its release step is idempotent, so pushing first does not conflict. Pre-release tags (`-rc.*`, `-alpha.*`, `-beta.*`, `-nightly.*`) skip GitHub-Release creation by design.
