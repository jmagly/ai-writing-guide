...
**Decision**: DELIVERED for opt-in backend, not approved for default switch
...
mutation, release tag approval, default-backend approval, or production
closeout.
...
| --- | --- | --- |
| Local implementation | DELIVERED | Opt-in Fortemi Core v2 export, sync/cache, query/discover/show, graph traversal, research-query, KB traversal, and Cockpit capability validation shipped on `origin/main`. |
...
| Tracker closeout | READY FOR OPT-IN CHILDREN | `tea login list` is empty, but the operator authorized Gitea MCP on 2026-07-02. Child issue closeout can use MCP with the pushed commit and CI evidence; default-switch filing remains gated. |
...
6. Keep the default-backend switch draft unfiled until #1691 is green in remote
   CI and the package boundary is resolved.

## Decision

The opt-in Fortemi Core backend migration is delivered to `origin/main` with
green remote CI. Production default-backend closeout remains blocked by package
boundary disposition and the separate default-switch issue.

## Next Validation

If any files change after this report, rerun the local pre-push gate and wait
for green remote CI again. Before filing the default-switch issue, update this
