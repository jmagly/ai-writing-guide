...
| OpenHuman | `.agents/agents/` | AGENTS.md bridge | `.openhuman/skills/` (kernel) + `.openhuman/.aiwg/skills/` (standard) | AGENTS.md `### Rule:` sections + `aiwg show rule` | `aiwg use sdlc --provider openhuman` |
...
- **OpenHuman** (tinyhumansai): OSS personal-AI runtime (Rust core + Tauri desktop), AIWG-convention-aware out of the box — ships `.agents/`, `AGENTS.md`, `.claude/`, `.codex/`. Markdown personas deploy to `.agents/agents/` (consumed by the coding hosts OpenHuman drives — `claude_code`/`factory`); kernel skills to `.openhuman/skills/` (project-scope native scan via `ops_discover.rs`, trust-marker gated), standard skills sequestered under `.openhuman/.aiwg/skills/`; commands and rules aggregate into AGENTS.md. The native agent harness (TOML specialists) is an opt-in Tier-2 surface (#1559). Induction epic #1552; see `docs/integrations/openhuman-quickstart.md` (#1558).
