...
8. **GitHub mirror is automatic** — the Gitea tag push in step 7 triggers `github-mirror.yml`, which pushes `main` + the tag to the GitHub mirror (`jmagly/aiwg`) **and** creates the GitHub Release using `docs/releases/v<version>-announcement.md` as the body (it reads the `GH_ACCESS_TOKEN` Gitea Actions secret). **Do NOT run `git push github` or `gh release create` manually for stable releases** — a manual tag push races the workflow's own tag push and makes the mirror job fail on "tag already exists." Pre-release tags (`-rc.*`, `-alpha.*`, `-beta.*`, `-nightly.*`) skip GitHub-Release creation by design.
