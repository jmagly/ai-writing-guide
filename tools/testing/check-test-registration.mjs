...
import { nodeFiles } from '../../config/test-lanes.mjs';
...
const report = { schemaVersion: 1, scope: 'Root test/, framework test/ and Ralph tool tests; fixtures excluded. App/package and VS Code extension tests have separate owners.', units: 'Source files and imported APIs, not registered or executed cases', totalFiles: records.length, files: records, errors };
...
if (errors.length) {
  ...
  process.exitCode = 1;
} else console.log(`Runner ownership: ${records.length} source files assigned; report ${path.relative(root, output)}`);
