...
export async function runOmpMcpConformance({ binary = process.env.AIWG_OMP_BIN || 'omp', outputFile } = {}) {
  ...
  try {
    ...
    await writeFile(fixture, `const fs=require('fs');const log=x=>fs.appendFileSync(${JSON.stringify(trace)},JSON.stringify(x)+'\\n');log({pid:process.pid});const tool=${JSON.stringify(tool)};require('readline').createInterface({input:process.stdin}).on('line',line=>{const r=JSON.parse(line);log({method:r.method});if(r.id===undefined)return;const result=r.method==='initialize'?{protocolVersion:r.params.protocolVersion,capabilities:{tools:{}},serverInfo:{name:'aiwg-stdio',version:'1'}}:r.method==='tools/list'?{tools:[tool]}:r.method==='tools/call'?{content:[{type:'text',text:'MCP_VERIFIED:'+r.params.arguments.text}]}:{};process.stdout.write(JSON.stringify({jsonrpc:'2.0',id:r.id,result})+'\\n');});process.on('SIGTERM',()=>{log({method:'disconnect'});process.exit(0)});process.stdin.on('end',()=>{log({method:'disconnect'});process.exit(0)});`);
