...
export async function isOverwriteSafe(filePath: string): Promise<boolean> {
  try {
    ...
    return head.includes(AIWG_SIGNATURE_COMMENT);
  } catch (err) {
...
export async function generate(opts: ContextPipelineOptions): Promise<ContextPipelineResult> {
  ...
  if (!opts.skip?.aiwgMd) {
    // Skeleton: AIWG.md emission is wired to the CLAUDE.md template copy in commit 3.
    // For the initial skeleton we leave this as a placeholder so callers get the
    // surface contract without committing to the full template copy until the wiring
    // PR. The placeholder still emits a minimal AIWG.md so AGENTS.md's link target
    // exists.
    ...
    const placeholder = [
      ...
      '',
      '<!--',
      '  Generated by AIWG. CLAUDE.md-shaped framework context for non-Claude providers.',
      '  Full content emission lands with the #1104 wiring PR; this is the skeleton stub.',
      '-->',
      '',
    ...
    if (canWrite) {
      await atomicWrite(aiwgMdPath, placeholder);
